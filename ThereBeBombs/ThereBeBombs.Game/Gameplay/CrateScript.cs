...
namespace ThereBeBombs.Gameplay
{
    public class CrateScript : SyncScript
    {
        ...
        public void UpdateAnimation()
        {
            ...
            animationTime += elapsedTotalSeconds * 8;
            double coinHeight = Math.Max(0, Math.Sin(animationTime));
            Entity.Transform.Position.Y = 1 + (float)coinHeight;

            float uniformScale = (float) Math.Max(0, Math.Min(1, (2 * Math.PI - animationTime) / Math.PI));
        ...
        protected void CollisionStarted()
        {
            ...
            int numCoins = 3 + rand.Next(4);

            for (int i = 0; i < numCoins; i++)
            {
                Vector3 offsetVector = new Vector3((float)rand.NextDouble() - 0.5f, (float)rand.NextDouble(), (float)rand.NextDouble() - 0.5f);
                effectMatrix = Matrix.Scaling(0.7f + (float)rand.NextDouble() * 0.3f) *
                    Matrix.Translation(Entity.Transform.WorldMatrix.TranslationVector + offsetVector * 2f);
